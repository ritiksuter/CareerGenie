        ...
        <DialogHeader>
          ...
          <DialogDescription>
            <div>
                ...
                <input type="file" accept="application/pdf" onChange={onFileChange} id="resumeUpload" className="hidden" />
            </div>
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
            <Button variant={'outline'}>Cancel</Button>
            <Button disabled={!file || loading} onClick={onUploadAndAnalyze}> {loading ? <Loader2Icon className="animate-spin" /> : <> <Sparkle /> Upload and Analyze </>}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
